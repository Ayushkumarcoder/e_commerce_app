...
const Login = () => {
  ...
  const{navigate,backednUrl,token, setToken} = useContext(ShopContext);
  ...
  const onSubmitHandler = async(event)=>{
      ...
      try {
        ...
        if(currentState === 'Sign Up'){
          ...
          const response = await axios.post(backednUrl + '/api/user/register', {name, email, password});
          ...
          const response = await axios.post(backednUrl + '/api/user/login', {email, password});
          if(response.data.success){
            setToken(response.data.token);
            localStorage.setItem('token', response.data.token);
          }else{
  ...
  return (
    <form onSubmit={onSubmitHandler} className='flex flex-col items-center w-[90%] sm:max-w-96 m-auto mt-14 gap-4 text-gray-800'>
      ...
      <input onChange={(e)=>setEmail(e.target.value)} value={email} type='email' className='w-full px-3 py-2 border border-gray-800 ' placeholder='Email Address' required></input>
